...
# <a name="deploying-the-azure-rights-management-connector"></a>部署 Azure Rights Management 連接器

>*適用對象︰Azure 資訊保護、Windows Server 2012、Windows Server 2012 R2*

使用此資訊了解 Azure Rights Management 連接器，以及如何為您的組織成功加以部署。 此連接器針對使用 Microsoft **Exchange Server**、**SharePoint Server**，或執行 Windows Server 和**檔案分類基礎結構** (FCI) 之檔案伺服器的現有內部部署提供資料保護。
...
## <a name="overview-of-the-microsoft-rights-management-connector"></a>Microsoft Rights Management 連接器概觀
Microsoft Rights Management (RMS) 連接器可讓您快速啟用現有的內部部署伺服器，以搭配使用其資訊版權管理 (IRM) 功能與雲端架構 Microsoft Rights Management 服務 (Azure RMS)。 利用此功能，IT 和使用者可輕鬆保護您組織內部和外部的文件和圖片，無需安裝其他基礎結構或與其他組織建立信任關係。 

RMS 連接器是個占用資源很少的服務，您可安裝於執行 Windows Server 2012 R2、Windows Server 2012 或 Windows Server 2008 R2 之伺服器的內部部署中。 除了在實體電腦上執行連接器，您也可以在包括 Azure IaaS VM 的虛擬機器上執行它。 部署連接器後，其會成為內部部署伺服器與雲端服務之間的通訊介面 (轉送)，如下圖所示。 箭頭表示網路連線的起始方向。

![RMS 連接器架構概觀](../media/RMS_connector.png)


### <a name="onpremises-servers-supported"></a>支援內部部署伺服器

RMS 連接器支援下列內部部署伺服器：Exchange Server、SharePoint Server，以及執行 Windows Server 的檔案伺服器，並使用檔案分類基礎結構來分類原則，且將原則套用至資料夾中的 Office 文件。 

> [!NOTE]
> 如果您想要使用檔案分類基礎結構保護所有檔案類型 (而不只是 Office 文件)，請勿使用 RMS 連接器，請改用 [RMS 保護 Cmdlet](https://msdn.microsoft.com/library/azure/mt433195.aspx)。

如需這些內部部署伺服器的支援版本，請參閱[支援 Azure RMS 的內部部署伺服器](..\get-started\requirements-servers.md)。


### <a name="support-for-hybrid-scenarios"></a>混合式案例的支援

在混合式案例中，即使某些使用者已連接到線上服務，您仍可使用 RMS 連接器。 例如，某些使用者的信箱使用 Exchange Online，而某些使用者的信箱使用 Exchange Server。 安裝 RMS 連接器之後，所有使用者皆可藉由使用 Azure RMS 保護並取用電子郵件和附件，而資訊保護會在兩個部署組態之間順暢地運作。

### <a name="support-for-customermanaged-keys-byok"></a>客戶管理金鑰的支援 (BYOK)

如果您針對 Azure RMS 管理自己的租用戶金鑰 (自備金鑰，即 BYOK 案例)，RMS 連接器和使用該連接器的內部部署伺服器不會存取含您租用戶金鑰的硬體安全性模組 (HSM)。 這是因為所有使用租用戶金鑰的密碼編譯作業都是在 Azure RMS 執行，而不是在內部。

如需深入了解此管理租用戶金鑰案例的詳細資訊，請參閱[規劃及實作 Azure Rights Management 租用戶金鑰](../plan-design\plan-implement-tenant-key.md)。

## <a name="prerequisites-for-the-rms-connector"></a>RMS 連接器的必要條件
...
## <a name="steps-to-deploy-the-rms-connector"></a>部署 RMS 連接器的步驟

連接器不會自動檢查成功部署所需的所有[必要條件](deploy-rms-connector.md#prerequisites-for-the-rms-connector)，因此請先確定這些必要條件已就緒，再開始進行。 部署會要求您安裝連接器、設定連接器，然後設定您想要用於此連接器的伺服器。 
...
-   **步驟 5：**[設定伺服器使用 RMS 連接器](configure-servers-rms-connector.md)
    ...
    -   [設定 Exchange 伺服器以使用連接器](configure-servers-rms-connector.md#configuring-an-exchange-server-to-use-the-connector)

    -   [設定 SharePoint 伺服器以使用連接器](configure-servers-rms-connector.md#configuring-a-sharepoint-server-to-use-the-connector)

    -   [為檔案分類基礎結構設定檔案伺服器以使用連接器](configure-servers-rms-connector.md#configuring-a-file-server-for-file-classification-infrastructure-to-use-the-connector)


## <a name="next-steps"></a>後續步驟

移至步驟 1：[安裝和設定 Azure Rights Management 連接器](install-configure-rms-connector.md)。
