...
此密碼有一些字元限制。 您無法使用下列任何字元的密碼 ︰連字號 (&)、左角括弧 ([)、右角括弧 (])、直引號 (") 和所有格號 (')。 如果您的密碼有任何這些字元，RMS 連接器的驗證將會失敗，您會看到錯誤訊息「該使用者名稱和密碼組合不正確」，儘管您在其他案例中可以使用此帳戶和密碼成功登入。 如果您的密碼是這種情形，則使用不同帳戶，其密碼沒有任何這些特殊字元，或者重設您的密碼，讓它不具有任何這些特殊字元。
...
>
> -   從網頁瀏覽器中，連線至 http://&lt;connectoraddress&gt;/_wmcs/certification/servercertification.asmx，以已安裝 RMS 連接器的伺服器位址或名稱取代 &lt;connectoraddress&gt;。 成功連接後會顯示 ServerCertificationWebService 頁面。
