---
title: "規劃及實作 Azure Rights Management 租用戶金鑰 | Azure 資訊保護"
...
使用本文資訊協助您規劃和管理 Azure 資訊保護租用戶金鑰。 例如，不是 Microsoft 管理租用戶金鑰 (預設值)，而是您可能想要管理您自己的租用戶金鑰，以遵循適用於貴組織的特定法規。 管理您自己的租用戶金鑰也稱為自備金鑰或 BYOK。

> [!NOTE]
> 相當於 Azure 資訊保護租用戶金鑰的內部部署，稱為伺服器授權人憑證 (SLC) 金鑰。 Azure 資訊保護為擁有 Azure 資訊保護訂用帳戶的每個組織維護一或多個金鑰。 只要組織內的 Azure 資訊保護使用金鑰時 (例如使用者金鑰、電腦金鑰、文件加密金鑰)，便會以加密編譯的方式鏈結至您的 Azure 資訊保護租用戶金鑰。

**概覽：** 使用下表做為您建議的租用戶金鑰拓撲的快速指南。 然後，如需詳細資訊，請使用其他文件。

如果您使用 Microsoft 管理的租用戶金鑰部署 Azure 資訊保護，則稍後可以變更為 BYOK。 不過，您目前無法將 Azure 資訊保護租用戶金鑰從 BYOK 變更為 Microsoft 管理的租用戶金鑰。

|商務需求|建議的租用戶金鑰拓撲|
|------------------------|-----------------------------------|
|快速部署 Azure 資訊保護且不需要特殊硬體|由 Microsoft 管理|
...
決定最適合您組織的租用戶金鑰拓撲。 根據預設，Azure 資訊保護會產生您的租用戶金鑰，並管理租用戶金鑰生命週期的大多數作業。 這是系統管理負擔最小的最簡單選項。 在大多數情況下，您甚至無需知道您擁有租用戶金鑰。 您只要申請 Azure 資訊保護，Microsoft 會為您處理金鑰管理程序的剩餘部分。
...
雖然是選用項，您也可能想要使用來自 Azure 資訊保護幾近即時的使用記錄，查看您的租用戶金鑰目前的實際使用方式與時間。

> [!NOTE]
> 作為其他防護措施，Azure 金鑰保存庫對其在北美、EMEA (歐洲、中東和非洲) 和亞洲等區域的資料中心使用不同的安全網域， 而對於 Azure 的不同執行個體，例如 Microsoft Azure 德國及 Azure Government 也是比照辦理。 當您管理自己的租用戶金鑰時，它會連結到您 Azure 資訊保護租用戶所登錄地區或執行個體的安全網域。 例如，歐洲客戶的租用戶金鑰無法在北美或亞洲的資料中心使用。
...
![Azure 資訊保護租用戶金鑰週期 - 預設由 Microsoft 管理](../media/RMS_BYOK_cloud.png)

第二張圖顯示當您管理自己的租用戶金鑰時所需執行的其他步驟。

![Azure 資訊保護租用戶金鑰週期 - BYOK 管理](../media/RMS_BYOK_onprem4.png)
...
> [!IMPORTANT]
> 如果您已經透過由 Microsoft 管理的租用戶金鑰開始使用 Azure 資訊保護，而現在您想要自行管理租用戶金鑰 (移至 BYOK)，您先前受保護的文件和電子郵件仍可使用備份金鑰存取。 不過，如果有使用者執行 Office 2010，在您執行這些程序之前，[請先連絡 Microsoft 支援服務](../get-started/information-support.md#to-contact-microsoft-support)。 這些電腦將需要一些額外設定步驟。
> 
...
|需求|詳細資訊|
|---------------|--------------------|
|支援 Azure 資訊保護的訂用帳戶。|如需可用訂用帳戶的詳細資訊，請參閱 Azure 資訊保護[定價頁面](https://go.microsoft.com/fwlink/?LinkId=827589)。|
...
|為金鑰保存庫 BYOK 所列出的所有必要條件。|請從 Azure 金鑰保存庫文件參閱 [BYOK 的必要條件](https://azure.microsoft.com/documentation/articles/key-vault-hsm-protected-keys/#prerequisites-for-byok)。 <br /><br />**注意**：如果您使用軟體金鑰對硬體金鑰從 AD RMS 移轉到 Azure 資訊保護，則 Thales 韌體的版本至少必須是 11.62 版。|
...
您現在可設定 Azure 資訊保護將此金鑰作為組織的 Azure 資訊保護租用戶金鑰使用。 請使用 Azure RMS Cmdlet，先連接到 Azure Rights Management Service，再登入：
...
1.  開始使用您的租用戶金鑰：
    ...
    -   若您尚未這麼做，則現在必須啟用 Rights Management Service，如此貴組織才能開始使用 Azure 資訊保護。 使用者會立即開始使用您的租用戶金鑰 (由 Microsoft 管理或由您在 Azure 金鑰保存庫中自行管理)。
